...
5. **Review in plan mode**, then on approval (create the folder if missing):
   - Write `.claude/workspace/reports/transfers/claude/{YYYY-MM-DD}-{topic}.md`, where
     `{topic}` names **what the session did** (e.g. `harden-hours-input-parsing`), **not**
     this skill's name.
...
3. **Re-ground before acting** (do NOT start new work first): read the brief's pointers + the
   recent `git log`, then run a **basic functional check** (`just build` then `just run` -
   both must exit 0 with no crash or hang) to catch drift or undocumented breakage. Building a
...
- Files: `src/movie-ticket.cpp:24-58`, `sample-inputs/movie-ticket.txt`
- Recent commits: `<hash> <subject>` (run `git log --oneline -5`)
- Branch: {branch} PR: #{id}
- Commands: `just list`, `just build <name>`, `just build-all`, `just run <name>`
...
- `sample-inputs\<name>.txt` in sync with each touched program's cin read order:
  {yes/no as of close}
