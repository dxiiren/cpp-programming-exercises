...
- {Step to bring the world up + how to check it's up + what to do if it's down. e.g.
  `pwsh ./setup.ps1` if `g++` (w64devkit) is missing; sanity gate: `just build-all` exits 0
  with `57/57 PASS`. Reference the exact command.}
...
- {Any project house-rule that applies, e.g. "keep `sample-inputs\<name>.txt` in sync with
  any cin read-order change in that program".}

## What counts as a REAL blocker

- **Real (may mark `BLOCKED`):** {code facts - a missing function at `file:line`, an input the
  app cannot receive, a toolchain feature genuinely unavailable in g++ 16; or a hard infra failure
  you tried once to fix and couldn't.}
- **NOT a blocker (keep working):** "it's late", "it's complex", "needs a focused session", missing
  fixture data (-> create a `sample-inputs\<name>.txt`), a stale `out\` build (-> `just clean` +
  rebuild, continue).
...
A finite, attended-ish goal - harden the three input-parsing weak spots in the CLI:

```markdown
# GOAL - Harden the three known input-parsing weak spots in assessment3-employee-payroll.cpp
...
Make the hours prompt, the stop prompt, and the id/name read sequence survive malformed input with
a clear re-prompt instead of a fail-stated stream (which today makes the entry loop spin forever).
Success = `just run assessment3-employee-payroll` still exits 0 on the committed
`sample-inputs\assessment3-employee-payroll.txt`, and each malformed-input case re-prompts
instead of hanging.

## Locked decisions (do not re-litigate)

- Keep the existing prompt wording and output format byte-for-byte for well-formed input.
- No new dependencies, no build system - standalone per-file `g++` compile stays.

## Environment bootstrap

- Sanity gate before starting: `just build assessment3-employee-payroll` and
  `just run assessment3-employee-payroll` both exit 0 (no crash or hang).

## Hard prohibitions / guardrails

- NEVER git commit/push/stage - leave changes in the working tree for review.
- No GitHub PR/issue posts.
- Touch only `src/assessment3-employee-payroll.cpp` (+ its `sample-inputs` file if a new
  prompt is added).

## What counts as a REAL blocker

- Real: a case that cannot be re-prompted without restructuring the whole read loop - cite the
  `file:line` and describe the restructure needed.
- Not a blocker: "the input handling is messy" - slice the cases and harden them one at a time.

## Definition of Done

Per item - `DONE` only when:

- [ ] The malformed case re-prompts (shown by a manual `just run-interactive
      assessment3-employee-payroll` transcript excerpt)
- [ ] `just run assessment3-employee-payroll` on the committed sample input still exits 0 with
      unchanged summary output
- [ ] Evidence pasted into the Run Log (exit codes + transcript lines)

Global: **G1** all three weak spots hardened. **G2** `just build-all` +
`just run assessment3-employee-payroll` green at the end.

## Work List

| #   | Item                                                                 | Status |
| --- | -------------------------------------------------------------------- | ------ |
| 1   | Hours prompt (`cin >> hour` fail-states on non-numeric input)        | `TODO` |
| 2   | Stop prompt (`cin >> answer` accepts any char; only `y` exits)       | `TODO` |
| 3   | Id/name newline dance (`cin >> id; cin.get(); getline(cin, name)`)   | `TODO` |
