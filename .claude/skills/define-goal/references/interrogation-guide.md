...
3. What proves **one** item is done? (Push for an evidence artifact - a pasted `just run` exit code
   + output excerpt, a transcript, a report path - not just a feeling.)
...
- What's the before->after transformation, stated concretely enough to apply uniformly (e.g.
  raw `cin >>` reads -> a validated read helper, or a naming convention pass)?
- Isolation: do items touch shared files (-> serialize) or disjoint ones (-> can parallelize)?
- What's the per-item verification (`just build` + `just run` on the committed sample input /
  `g++ -Wall -Wextra` clean / grep) that proves the change is safe?
