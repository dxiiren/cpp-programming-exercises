...
`github.com/dxiiren/cpp-programming-exercises`.
...
- If there are multiple commits, synthesize one Conventional title that summarizes
  the change, e.g. `fix(src): guard the movie-ticket price prompt against non-numeric input`
  or `docs(tooling): document the sample-inputs format`. Pick the type/scope from the
...
- <how it was verified: `just build-all` summary line + `just run <name>` exit codes /
  output excerpt, manual `just run-interactive <name>` check, etc.>
```
...
title used. This repo has no CI — verification is the `just build-all` / `just run <name>`
...
| `/pre-pr-review` | Optional self-review (C++ collection checklist) before opening the PR         |
