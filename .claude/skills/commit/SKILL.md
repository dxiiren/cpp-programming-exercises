...
2. Run **Step 0** (clear stale lock), then go straight to `git add -A` -> `git commit`
   ...
   2-second `git status` scan and exclude only those. Build output `out/` is git-ignored
   already; no compiled binaries are tracked in this repo.)
...
scope: src, input, tooling, docs, skills
```
