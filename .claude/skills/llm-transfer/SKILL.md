...
## 1. Role
You are {persona - e.g. a senior C++ engineer / a CLI-UX reviewer / an algorithms tutor}.
...
situation: what the project is (a collection of 57 standalone plain-C++ console exercises
from a university programming course), the domain, and WHY this task matters. Self-contained -
...
`src/assessment3-employee-payroll.cpp` (excerpt)
```
...
## 6. Constraints & Rules
- Tech stack: {plain C++ (g++ / w64devkit), no build system, no dependencies; run via `just`}
...
- **Selective, not a dump.** Include only artifacts that bear on the task; summarize the rest.
  ...
  windows can be small). In this repo every program is one small standalone file
  (`src/*.cpp`, roughly 20-150 lines) - inlining the relevant program(s) whole is usually
  right; never dump all 57.
...
Target: _"Hand off an input-parsing bug to an external LLM for a fix."_

````text
# MASTER PROMPT - Guard the hours prompt against non-numeric input

## 1. Role
You are a senior C++ engineer who writes precise, minimal fixes.

## 2. Mission
Stop `src/assessment3-employee-payroll.cpp` from looping forever when the user types a
non-number at the hours prompt, without changing any other prompt's behavior.

## 3. Background & Context
One program from a 57-exercise plain-C++ uni collection: a console app that computes each
employee's weekly gross/net pay from hours worked and prints a closing summary. It reads
every value from `cin`. No repo access - everything you need is below.

## 4. Current State
- Done: the app runs end-to-end with well-formed input.
- Failing: a non-numeric entry at the hours prompt fail-states `cin`; every later `cin >>`
  (including the "Do you want to stop ?" read) silently fails, so the entry loop never sees a
  `y` and spins forever.
...
cin>>answer;
```
Failure: no exception - the loop re-prints the employee prompts endlessly with garbage values.

## 6. Constraints & Rules
- Plain C++, no new dependencies. Keep the prompt text unchanged.

## 7. Your Task
Return the corrected read (clear the stream and re-prompt until a valid number is entered) and
a one-paragraph explanation.

## 8. Output Format & Success Criteria
- Output: the fixed snippet only + the explanation.
- Done: non-numeric input re-prompts; valid numbers still accepted; the committed
  `sample-inputs/assessment3-employee-payroll.txt` run is byte-identical.

## 9. Open Questions & Assumptions
- Assume negative hours are out of scope (matching current behavior).

## Reminder
Return the fixed hours-prompt read + a one-paragraph explanation. No other behavior changes.
````
