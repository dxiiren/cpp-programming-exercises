...
# C++ Programming Exercises Bootstrap Setup
#
...
Write-Host ""
Write-Host "C++ Programming Exercises Bootstrap Setup" -ForegroundColor Cyan
...
# ---------- 6. w64devkit (portable GCC toolchain) ----------
# Portable zip to a pinned path -- no MSYS2 interactivity, no PATH pollution beyond one dir.
$gccDir = Join-Path $env:LOCALAPPDATA "Programs\w64devkit"
$gppExe = Join-Path $gccDir "bin\g++.exe"
Refresh-Path
if (Test-Path $gppExe) {
    Write-Host "[OK] w64devkit already installed: $(& $gppExe --version 2>&1 | Select-Object -First 1)" -ForegroundColor Green
} else {
    Write-Host "[INSTALL] Downloading w64devkit (latest release)..." -ForegroundColor Yellow
    $savedEAP = $ErrorActionPreference
    $ErrorActionPreference = 'Continue'
    $url = & gh api repos/skeeto/w64devkit/releases/latest --jq '.assets[] | select(.name | test("x64-.*\\.exe$")) | .browser_download_url' 2>&1 | Select-Object -First 1
    $ErrorActionPreference = $savedEAP
    if ($url) {
        try {
            $sfx = Join-Path $env:TEMP "w64devkit-sfx.exe"
            Invoke-WebRequest -Uri $url -OutFile $sfx -UseBasicParsing
            & $sfx -o"$env:LOCALAPPDATA\Programs" -y | Out-Null
            Remove-Item $sfx -Force -ErrorAction SilentlyContinue
            if (Test-Path $gppExe) {
                Write-Host "[OK] w64devkit installed: $(& $gppExe --version 2>&1 | Select-Object -First 1)" -ForegroundColor Green
            } else {
                Write-Host "[FAIL] w64devkit extract succeeded but g++.exe missing at $gppExe" -ForegroundColor Red
            }
        } catch {
            Write-Host "[FAIL] w64devkit download/extract failed: $_" -ForegroundColor Red
        }
    } else {
        Write-Host "[WARN] Could not resolve w64devkit release URL (is gh authenticated?). Install manually from https://github.com/skeeto/w64devkit/releases" -ForegroundColor Yellow
    }
}
...
# g++ lives at a pinned path (never on PATH) -- verify by path, not by command.
if (Test-Path $gppExe) {
    Write-Host "  [OK] g++ (w64devkit, pinned path)" -ForegroundColor Green
} else {
    Write-Host "  [MISSING] g++ (w64devkit at $gppExe)" -ForegroundColor Red
    $missing += 'g++'
}
...
Write-Host "  1. List the programs:      just list" -ForegroundColor Gray
Write-Host "  2. Compile everything:     just build-all" -ForegroundColor Gray
Write-Host "  3. Run one program:        just run movie-ticket    (your own input: just run-interactive <name>)" -ForegroundColor Gray
